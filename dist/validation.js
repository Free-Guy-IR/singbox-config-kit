...
function validateListenPort(value, path) {
...
export function validateSingBoxCoreConfig(input) {
    const parsed = rawSingBoxCoreConfigSchema.safeParse(input);
    if (!parsed.success) {
        return {
            ok: false,
            issues: parsed.error.issues.map(zodIssue => issue(pathForZod(zodIssue.path.filter((part) => typeof part === "string" || typeof part === "number")), "SB_SCHEMA_INVALID_CORE_CONFIG", zodIssue.message))
        };
    }
    const issues = [];
    let config;
...
export function assertValidSingBoxCoreConfig(input) {
...
export function isHysteria2Inbound(value) {
    return !!value && typeof value === "object" && value.type === "hysteria2";
}
